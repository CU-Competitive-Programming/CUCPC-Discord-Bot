...
commands = {
    ...
    "!submit": "Select your file and write this as a comment to submit your bot, you this in the **season-"
    + settings.season
    + "** to submit for the tournament",
...
adminCommands = {
    "!subs": "!subs True/False opens or closes submissions",
...
async def on_ready():  # startup
    ...
    try:
        haliteBackup = discord.utils.get(
            client.get_all_channels(),
            server__name=settings.serverName,
            name="halite-backup",
        )
    ...
    embed.add_field(
        name="Currently on Season-" + settings.season,
        value="Check the schedule for more info!",
        inline=False,
    )
    ...
    await client.change_presence(
        game=discord.Game(name="Running Season-" + settings.season + " | !help")
    )
...
async def on_message(message):
    ...
    try:
        if message.content.startswith("!submit"):
            ...
            if (
                not settings.submit and isPlayer and not settings.opened
                ...
                if (
                    str(message.channel) != "season-" + settings.season
                    and str(message.channel) != "battles"
                ...
                await client.send_message(
                    message.channel,
                    "**You are not a Player! Sign up for the tournament first!** "
                    + message.author.mention,
                )
            ...
            embed = discord.Embed(
                title="Submissions for Season-" + settings.season,
                description=desc,
                color=0xFFAE00,
            )
            embed.set_thumbnail(
            ...
            if settings.onTour and str(message.channel) == "battles":
                try:
                    ...
                    try:
                        ...
                        await client.send_message(
                            message.channel, "*Using default size map : 240x160*"
                        )
                        width = "240"
                        height = "160"
                    ...
                    await client.send_message(
                        message.channel,
                        "*Running battle...* " + settings.emojis["logo"],
                    )
                    status, result, logs, replay = await funcs.battle(
                        pp, width, height, mode
                    )
            ...
            for m in members:
                if player in m.roles:
                    p = settings.db.players.find_one(
                        {"username": str(m).replace(" ", "")}
                    )
                    if p is None:
            ...
            if len(players) > 0:
                text = (
                    "**Here are all the players in this season :** "
                    + settings.emojis["aspiring"]
                    + "\n"
                )
                ...
                text = (
                    "**No registered player at the moment!** "
                    + settings.emojis["aspiring"]
                )
            ...
            if settings.onTour:
                try:
                    ...
                    await client.send_message(
                        message.channel,
                        "**Brackets are not up yet!** " + settings.emojis["paper"],
                    )

            else:  # if no tournament is running
                await client.send_message(
                    message.channel,
                    "**No tournament currently ongoing!** "
                    + settings.emojis["explosion"],
                )
            ...
            embed.add_field(
                name="PayPal",
                value="Donations are used to help support Halite Tournaments. We use your contributions to run our servers and give cash prizes.\nDonate here: https://www.paypal.me/HaliteTournaments.\nDonating will give you the **Contributor** role which has access to the Contributors voice channel. More privileges for Contributors will be coming!\n"
                + settings.emojis["paper"],
                inline=False,
            )
            embed.add_field(
                name="GitHub",
                value="You can also contribute by working with us on our repositories! Check it out at : https://github.com/HaliteTournaments",
                inline=False,
            )
            ...
            if message.content.startswith("!type"):  # make bot type in current channel
                ...
                if settings.onTour:
                    try:
                        ...
                        await client.send_message(
                            message.channel,
                            "*Running match...* " + settings.emojis["logo"],
                        )
                        if haliteVegas != None:
                            text = "!create"
                            if mode == 3:
                ...
                if s != "":
                    try:
                        ...
                        settings.submit = boo
                        await client.send_message(
                            message.channel, "**Setting : " + s[0] + " in submissions**"
                        )
                ...
                try:
                    ...
                    if s != "":
                        ...
                        await client.send_message(
                            message.channel,
                            "**Setting : " + s[0] + " in opened submissions**",
                        )
                ...
                if s != "":
                    ...
                    settings.onTour = boo
                    await client.send_message(
                        message.channel, "**Setting : " + s[0] + " in onTour**"
                    )
                ...
                try:
                    ...
                    await client.send_message(
                        message.channel, "**Error while uploading the brackets**"
                    )
                ...
                if t != "":
                    ...
                    settings.timeSub = t
                    await client.send_message(
                        message.channel, "**Setting : " + t + " in timeSub**"
                    )
                ...
                try:
                    ...
                    if r == "clear":
                        ...
                        settings.db.settings.update(
                            {}, {"$set": {"matches": {"placeholder": ["placeholder"]}}}
                        )
                        await client.send_message(
                            message.channel, "**Cleared schedule successfully**"
                        )
                        ...
                        settings.db.settings.update_one(
                            {}, {"$set": {"matches": settings.matches}}, upsert=True
                        )
                        await client.send_message(
                            message.channel, "**Scheduled match successfully**"
                        )
...
async def on_member_join(member):
    ...
    await client.send_message(
        ...
        "Welcome "
        + member.mention
        + " to Halite Tournaments! Check out the section "
        + channel.mention
        + " for information about the upcoming tournaments! "
        + settings.emojis["logo"],
    )
